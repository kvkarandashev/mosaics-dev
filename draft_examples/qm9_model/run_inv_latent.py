...
def resample():
    ...
    for res in latent_space_results:
        # try:
        CURR_RESULTS = []
        SMILES_init, X_init, y_ref, T_init, y_pred, y_goal, y_opt, T_opt, X_opt = (
            res[0],
            res[1],
            res[2],
            res[3],
        ...
        MOLS, D = inversion_potentials.chemspacesampler_inv_latent(
            SMILES_init, T_init, T_opt, params=params
        )
        y_sampled = []
        for mol in MOLS:
            X_sampled, _, _ = chemspace_potentials.initialize_from_smiles(
                mol, nBits=params["nBits"]
            )
            y_pred_ML = scalar_values.inverse_transform(
                selected_model.predict(scalar_features.transform(X_sampled))
            )[0][0]
