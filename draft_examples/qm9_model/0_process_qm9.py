...
from mosaics.minimized_functions.representations import generate_bob
...
def read_xyz(path):
    ...
    with open(path, "r") as file:
        ...
        for atom in lines[2 : n_atoms + 2]:
            ...
            except ValueError:  # some values in QM9 XYZ files are in a weird format.
                coordinates.append(
                    [
...
if __name__ == "__main__":
    ...
    GAP = (
        qm9_df["gap"].values * ha2kcalmol
    )  # qm9_df['h298_atom'].values #atomization energy in kcal/mol
    ...
    np.savez_compressed(
        "/data/jan/calculations/BOSS/qm9_processed.npz", X=X, y=y, SMILES=SMILES_SET
    )
