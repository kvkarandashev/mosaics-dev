...
def chemspacesampler_inv_ECFP(smiles_init, X_target, params=None):
    ...
    if params['strategy'] == "default":
        ...
        if params['NPAR'] == 1:
            chemspace_potentials.mc_run(egc_0,min_func,"chemspacesampler", respath, f"results_{0}", params)
        else:
            Parallel(n_jobs=params['NPAR'])(delayed(chemspace_potentials.mc_run)(egc_0,min_func,"chemspacesampler", respath, f"results_{i}", params) for i in range(params['NPAR']) )
        ana = chemspace_potentials.Analyze_Chemspace(respath+f"/*.pkl",rep_type="2d" , full_traj=False, verbose=False)
        ...
        for n in N_budget:
            ...
            respath = tempfile.mkdtemp()
            if params['NPAR'] == 1:
                chemspace_potentials.mc_run(egc_best,min_func,"chemspacesampler", respath, f"results_{0}", params)
            else:
                Parallel(n_jobs=params['NPAR'])(delayed(chemspace_potentials.mc_run)(egc_best,min_func,"chemspacesampler", respath, f"results_{i}", params) for i in range(params['NPAR']) )
            ana = chemspace_potentials.Analyze_Chemspace(respath+f"/*.pkl",rep_type="2d" , full_traj=False, verbose=False)
            _, GLOBAL_HISTOGRAM, _ = ana.parse_results()
            MOLS, D  = ana.post_process(GLOBAL_HISTOGRAM, X_target, params )
            shutil.rmtree(respath)
        ...
        for d, n in zip(d_arr, N_budget):
            ...
            respath = tempfile.mkdtemp()
            if params['NPAR'] == 1:
                chemspace_potentials.mc_run(egc_rep[0],min_func,"chemspacesampler", respath, f"results_{0}", params)
            else:
                Parallel(n_jobs=params['NPAR'])(delayed(chemspace_potentials.mc_run)(egc_rep[i],min_func,"chemspacesampler", respath, f"results_{i}", params) for i in range(params['NPAR']) )
            ana = chemspace_potentials.Analyze_Chemspace(respath+f"/*.pkl",rep_type="2d" , full_traj=False, verbose=False)
            _, GLOBAL_HISTOGRAM, _ = ana.parse_results()
            MOLS, D  = ana.post_process(GLOBAL_HISTOGRAM, X_target, params )
            shutil.rmtree(respath)
...
class potential_inv_latent:
    ...
    def __call__(self, trajectory_point_in):
        ...
        X_test   = chemspace_potentials.extended_get_single_FP(rdkit_mol, nBits=self.nBits).reshape(1,-1)
