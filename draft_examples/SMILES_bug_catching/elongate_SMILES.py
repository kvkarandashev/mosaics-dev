...
def SMILES_length(tp_in):
    rdkit_obj, SMILES = chemgraph_to_canonical_rdkit(
        tp_in.chemgraph(), print_all_SMILES_prefix=SMILES_print_prefix
    )
    return -len(SMILES)
...
for i, cur_cand in enumerate(rw.saved_candidates[:num_printed_saved_candidates]):
    ...
    print("SMILES:", chemgraph_to_canonical_rdkit(cur_cand.tp.chemgraph())[1])
